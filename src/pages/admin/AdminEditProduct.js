...
const AdminEditProduct = {
	async render () {
		try {
			...
			const { data: { categoryId, shortDescription, description, slug, name, sku, image, } } = await ProductApi.get(id)

			const { data: attributes } = await ProductApi.getAllAttributes()
			// console.log(attributes);
			const categoriesHTML = categories.map((cate) => {
				return `
          <option value="${cate.id}" ${categoryId === cate.id? `selected`:``}  >${cate.name}</option>
        `
			}).join('')
	      ...
	      `
			...
			const resultGetHTML = sku.map(item => {
				let valueHTML = ``;
				for (const [key, value] of Object.entries(item)) {
					valueHTML += `
						<input type="${key === 'color'?`color`: `text`}" value="${value}" name="${key}">
					`
				}
				return `
	...
	async afterRender () {
		...
		form.onSubmit = async ({
			...
			let img = document.querySelector('#formEdit img').src
			const file = image[0]
			if (file) {
				const storageRef = firebase.storage().ref(`images/${file.name}`)
				storageRef.put(file).then(async () => {
					console.log('ok')
					img = await storageRef.getDownloadURL().then((url) => {
						return url
					})
				})
			} else {
		...
		btnGetAdvanced.addEventListener('click', (e) => {
			...
			resultGet.innerHTML = arrayListBeforeRender.map(item => {
				// console.log(item)
				return `
          <div class="flex items-center">
            ${item.map(x => {
